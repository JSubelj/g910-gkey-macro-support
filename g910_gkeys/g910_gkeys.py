...
def reload_handler(sig, frame):
    log.info(f"Got signal, {signal.Signals(sig).name} reload g910-gkeys.")
    config.update_config()


def change_profile(dev: HIDDevice, profile: str):
...
def main():
    ...
    if args.create_config:
        device = HIDDevice()  # init usb device and keyboard interface
